...
const ProductCard = ({ items, setSearchText }) => {
  ...
  const percentage =
    ((items?.regularPrice - items?.discountedPrice) / items?.regularPrice) *
    100;
  ...
  return (
    <div className=" border border-gray-200 rounded-lg p-1 overflow-hidden hover:border-black duration-200 cursor-pointer">
      ...
      <div className=" flex flex-col gap-2 px-2 pb-2">
        ...
        <h2 className=" text-lg font-bold line-clamp-2">{items?.name}</h2>
                ...
                <DialogPanel className="w-full max-w-md rounded-xl bg-black backdrop-blur-2xl z-50 p-6">
                  ...
                  <p className="mt-2 text-sm/6 text-white/50">
                    ...
                    <span className="text-skyText">
                      <FormattedPrice
                        amount={items?.regularPrice - items?.discountedPrice}
                      />{" "}
                    </span>
                    from this product.
                  </p>
